...
document.addEventListener("DOMContentLoaded", function () {
    ...
    reservasLink.onclick = async function () {
        ...
        const btnsD = tableBody.querySelectorAll('.btn.btnEliminarReserva')
        eliminarReserva(btnsD)
    ...
    reservas.forEach(reg => {
        if (reg.estado != 'Finalizada') {
            body += `<tr>
                        ...
                        <td>${reg.detalle}</td>
                        <td nowrap row">
                            <button class="btn btn-primary btnFinalizar" id="${reg.codreserva}" title="Finalizar"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                            <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425z"/>
                            </svg></button>
                            <button class="btn btn-warning btnEditarReserva" id="${reg.codreserva}" title="Editar"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
...
const eliminarReserva = (btns) => {
    btns.forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            const { fetchedData } = await fetchData('reserva', { codreserva: reservaID });
            const room = fetchedData[0].fk_idhabitacion;
